...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($fecha_inicio && $fecha_fin && $id_cobrador) {
        ...
        $stmt->bind_param("iss", $id_cobrador, $fecha_inicio, $fecha_fin);  // Asegúrate que los tipos son correctos (i = int, s = string)
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $pagos[] = $row;
                // Acumulamos los montos por tipo de pago
        ...
        $mensaje = "Por favor, ingrese un rango de fechas válido y seleccione un cobrador.";
    }
}
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" action="reporte_pagos.php">
